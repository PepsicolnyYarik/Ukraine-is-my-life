...
int main()
{
	...
	while (1)
	{
		...
		if (temp == '\n')
		{ // переход на новую строчку = выход
			break; // выход тут
		}
		mas = (char*)realloc(mas, (j + 1)*sizeof(char)); //не забываем увеличить массив для ведения переменной
	...
	mas[n - 1] = '.'; //ставим в конце точку, чтобы выглядело ice
	for (i = 0, t = 0; i < n; i++, t++)
	{
		mas2[t] = mas[i];               //переписываем все во второй массив по условию задания
	}
	tolower(mas2, n);            //переводим буквы в нижний регистр для удовлетворения глаз
	tohigher(mas2, 0);
...
void tolower(char* text,int n)
{
	for (int i = 0; i < n; i++)
	{
		if (text[i] >= 'A' && text[i] <= 'Z')         //перевод букв из верхнего в нижний регистр
			text[i] += 'z' - 'Z';
